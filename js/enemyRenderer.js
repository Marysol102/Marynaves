...
export function drawEnemy(ctx, enemy) {
  ...
  try {
    ...
    // Flash blanco — sin ctx.filter (no compatible en todos los navegadores)
    if (enemy.flashTimer > 0) {
      ctx.globalAlpha = 0.55;
      ctx.fillStyle   = 'white';
      ctx.beginPath();
      ctx.arc(0, 0, enemy.size * 1.3, 0, Math.PI * 2);
      ctx.fill();
    }
    ...
    console.warn('drawEnemy error:', enemy.type, err.message);
  } finally {
    ctx.restore(); // siempre, pase lo que pase
  }
}
...
function drawDrifter(ctx, enemy, def) {
  ...
  for (let i = -3; i <= 3; i++) {
    const tx     = i * s * 0.22;
    const wiggle = Math.sin(now * 0.003 + i * 1.2) * s * 0.35;
...
function drawShell(ctx, enemy, def) {
  ...
  for (let t = 0; t < Math.PI * 2.5; t += 0.15) {
    const r  = Math.max(0, s * 0.6 * (1 - t / (Math.PI * 3)));
  ...
  ctx.strokeStyle = `hsla(${def.hue},100%,80%,0.85)`;
  ctx.lineWidth   = 3.5;
  ...
  for (const a of [-halfArc, halfArc]) {
    ...
    ctx.beginPath();
    ctx.arc(Math.cos(a) * s * 1.45, Math.sin(a) * s * 1.45, 3, 0, Math.PI * 2);
    ctx.fill();
  }
  ctx.restore();
}
...
function drawLeviathan(ctx, enemy, def) {
  ...
  for (const side of [-1, 1]) {
    for (let i = 0; i < 3; i++) {
      ...
      ctx.lineWidth   = 1.5;
      ctx.beginPath();
      ctx.moveTo(side * s * 0.65, sy);
      ctx.lineTo(side * (s * 0.95 + i * 4), sy - s * 0.12);
      ctx.stroke();
    }
  }
...
export function drawEnemyBullets(ctx, enemyBullets) {
  for (const b of enemyBullets) {
    try {
      if (b.isMine) drawMine(ctx, b);
      else          drawEnemyBullet(ctx, b);
    } catch (err) {
...
function drawEnemyBullet(ctx, b) {
  ...
  ctx.shadowBlur  = 10;
  const g = ctx.createRadialGradient(0, 0, 0, 0, 0, b.size || 4);
